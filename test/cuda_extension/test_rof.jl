...
@testset "CUDA ROF Single-Image Parity" begin
    ...
    prob_cpu = TotalVariationImageFiltering.TVProblem(
        f_cpu;
        lambda = 0.15f0,
        tv_mode = TotalVariationImageFiltering.IsotropicTV(),
    )
    u_cpu, stats_cpu = TotalVariationImageFiltering.solve(prob_cpu, config)

    f_gpu = CUDA.cu(f_cpu)
    prob_gpu = TotalVariationImageFiltering.TVProblem(
        f_gpu;
        lambda = 0.15f0,
        tv_mode = TotalVariationImageFiltering.IsotropicTV(),
    )
    u_gpu, stats_gpu = TotalVariationImageFiltering.solve(prob_gpu, config)
