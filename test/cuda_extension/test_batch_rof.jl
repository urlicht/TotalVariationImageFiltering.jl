...
@testset "CUDA ROF Batch API" begin
    ...
    u_batch_cpu, stats_batch_cpu =
        TotalVariationImageFiltering.solve_batch(f_batch_cpu, config; lambda = 0.15f0)

    f_batch_gpu = CUDA.cu(f_batch_cpu)
    u_batch_gpu, stats_batch_gpu =
        TotalVariationImageFiltering.solve_batch(f_batch_gpu, config; lambda = 0.15f0)
    u_sliced_gpu = similar(f_batch_gpu)
    @views for b = 1:size(f_batch_gpu, 3)
        prob_slice = TotalVariationImageFiltering.TVProblem(
            selectdim(f_batch_gpu, 3, b);
            lambda = 0.15f0,
            tv_mode = TotalVariationImageFiltering.IsotropicTV(),
        )
        u_slice, _ = TotalVariationImageFiltering.solve(prob_slice, config)
